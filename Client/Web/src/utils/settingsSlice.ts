...
import { userSettings, PaymentSettings, UserAccountSettingsType } from "./ObjectTypes";
...
const updateSettings = createAsyncThunk("settings/update", async (data: FormData) => {
    return await httpclient.post("http://127.0.0.1:5000/settings/update", data )
...
const getAccountSettings = createAsyncThunk("settings/account", async () => {
    ...
    return await httpclient.get("http://127.0.0.1:5000/settings/account" )
    .then( response => { return response })
})
...
const getPaymentSettings = createAsyncThunk('settings/payment', async () => {
    ...
    return await httpclient.get('http://127.0.0.1:5000/settings/payment')
    .then( response => { 
        console.log( response )
        return response })})

const getUserSettings = createAsyncThunk('settings', async () => {
    return await httpclient.get('http://127.0.0.1:5000/settings')
    .then( response => { return response })
})
export const SettingsSlice = createSlice({
    ...
    extraReducers: ( builder: any ) => {
        builder.addCase( updateSettings.pending, (state: userSettings, action: any ) => { console.log( 'pending ')})
        builder.addCase( updateSettings.rejected, (state: userSettings, action: any) => { console.log( 'rejected ')})
        builder.addCase( updateSettings.fulfilled, (state: userSettings, action: any ) => { console.log( 'fullfilled')})
        ...
        builder.addCase( getAccountSettings.fulfilled, (state: userSettings, action: any) => ( 
            action.payload != undefined && window.localStorage.setItem('accountSettings', JSON.stringify(action.payload.data)) 
        ))
        
        builder.addCase( getPaymentSettings.pending, (state: userSettings, action: any) => ( console.log( "pending")))
        builder.addCase( getPaymentSettings.rejected, (state: userSettings, action: any) => ( 
            window.localStorage.removeItem('accountSettings') ))
        builder.addCase( getPaymentSettings.fulfilled, (state: userSettings, action: any) => ( 
            action.payload != undefined && window.localStorage.setItem('paymenttSettings', JSON.stringify(action.payload.data)) 
        ))
    }
})
