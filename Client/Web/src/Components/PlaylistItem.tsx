...
import { FaPause, FaRegEdit } from 'react-icons/fa'
...
export default function PlaylistItem(props: {
  item: AudioItemPropType
  index: number
  setModalForm: React.Dispatch<React.SetStateAction<string>>,
  setSelectedTrack: React.Dispatch<React.SetStateAction<AudioItemPropType>>
  ...
  const audioItem = props.item as AudioItemPropType
  ...
  return(
    ...
    style={audioPlayer.nowPlaying.id == audioItem.id ? activeTrackStyle : {}}>
      ...
      <div className="track_item_container" style={optionSlider ? sliderOpen : {}}>
        <div className='track_item'onClick={() => {dispatch(play(audioItem))}}>
          {audioPlayer.nowPlaying.id == audioItem.id && audioPlayer.player.isPlaying ? 
          <>
            <span className='track_numner'>{props.index + 1} </span> 
            <button style={{'color': 'rgba(198, 161, 104,1)' }}><FaPause/></button> 
          </> : 
            <span className='track_numner'>{props.index + 1} </span>}
        ...
        <div className="track_buttons">
          {
            auth.user &&
            props.item.author_id != auth.user.id && 
            <span className='playlist_item_buy_button' onClick={() => props.openPurchaseModal(audioItem)}>Buy</span>
          }
